    ...
    return (
      <Drawer open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <DrawerContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="w-full bg-sidebar p-0 text-sidebar-foreground border-x-0 border-b-0 border-t border-border before:hidden rounded-t-xl overflow-hidden !h-[80dvh] flex flex-col focus-visible:outline-none"
          style={
            {
              "--sidebar-width": "100%",
            } as React.CSSProperties
        ...
        >
          ...
          <div className="flex h-full w-full flex-col pb-8">{children}</div>
        </DrawerContent>
      </Drawer>
    )
  }
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-[10px] font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
