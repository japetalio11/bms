...
export const messageRepository = {
  async uploadAttachment(
    ...
    const fileSize =
      data.fileSize || data.file_size || `${(file.size / 1024).toFixed(1)} KB`
    ...
    if (syncEngine.isNetworkOnline()) {
      try {
        ...
        if (Array.isArray(remoteList)) {
          ...
          const localById = new Map<string, LocalMessage>(
            localList.map((m) => [m.id, m])
          )

          const isStaff =
            currentUserObj?.role && currentUserObj.role !== "Mother"

          const formattedRemote: LocalMessage[] = remoteList
            .filter(
              (msg: any) => !pendingIds.has(msg.message_id || msg._id || msg.id)
            )
            .map((msg: any) => {
              ...
              return {
                ...
                message_date: msg.message_date
                  ? new Date(msg.message_date).toISOString()
          ...
          try {
            ...
            console.warn(
              "[messageRepository] Failed to update local Dexie cache:",
              dbErr
            )
          }
        ...
        console.warn(
          "[messageRepository] Backend fetch failed, returning cached messages:",
          apiErr
        )
      }
    }
  ...
  async sendMessage(payload: {
    ...
    if (syncEngine.isNetworkOnline()) {
      try {
        ...
        const canonicalId =
          created?.message_id || created?._id || created?.id || tempId
        ...
        console.warn(
          "[messageRepository] Server error, queuing offline message mutation:",
          err
        )
      }
    }
  ...
  async markAsRead(contactId: string): Promise<void> {
    try {
      ...
      if (syncEngine.isNetworkOnline()) {
        await apiClient
          .put("/api/v1/message/markAllAsRead", { sender_id: contactId })
          .catch(() => {})
      }
    } catch (err) {
